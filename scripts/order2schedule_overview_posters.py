...
for date in cs.dates:
    day, num, year = date
    path = os.path.join(args.output_dir, '%s-Poster-overview.tex' % (day))
#     out = open(path, 'w')
...
#     print >>out, '\\begin{SingleTrackSchedule}'
    for timerange, events in sorted(cs.schedule[date].iteritems(), cmp=sort_times):
        ...
        if len(events) >= 3:
            print 'this is a parallel session; hence skip'
                        ...
                        for p in event.papers:
                            if p.time is None:
                                if p.id is None:
                                    ...
                                    print ' No id and no time for the paper %s'  % ( p.title )
                                else:
