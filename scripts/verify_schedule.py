...
def parse_star(i, line):
    ...
    '''
    print ' SW: inside parse_star with line =%s=' %line
    content = line.split(' ',1)[1]
    print ' SW: inside parse_star with content =%s=' %content
    try:
        day, date, year = content.split(', ')
        #print ' SW: inside parse_star with day date year =%s=    '  %day
        #print ' SW: inside parse_star with day date year =%s=    '  %date
        #print ' SW: inside parse_star with day date year =%s=    '  %year
        month, date = date.split(' ') 
        #print ' SW: inside parse_star with month date  =%s=    '  %month
        #print ' SW: inside parse_star with day date =%s=    '  %date
    except ValueError:
        #print 'SW: THROWING ERROR !!!!!!'
    ...
    elif year != '2016':  #NOTE: this will need to change every year
